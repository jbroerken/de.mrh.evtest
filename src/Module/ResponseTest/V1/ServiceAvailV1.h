...
#ifndef ServiceAvailV1_h
#define ServiceAvailV1_h
...
class ServiceAvailV1 : public EventModule
{
public:
    ...
    ServiceAvailV1(bool& b_EventFailed,
                   std::string const& s_CategoryName,
                   MRH_Uint32 u32_SendEvent,
                   MRH_Uint32 u32_ResponseEvent) noexcept : EventModule("ServiceAvailV1",
                                                                        b_EventFailed),
...
private:
    ...
    void _HandleEvent(const MRH_Event* p_Event) override
    {
        ...
        else if (c_Data.u8_Available == MRH_EvD_Base_Result::MRH_EVD_BASE_RESULT_FAILED)
        {
            MRH::AB::ModuleLogger::Singleton().Log(GetName(), s_CategoryName + " service not available!", 
                                                   "ServiceAvailV1.h", __LINE__);
            SetEventFailed();
        }
        else
