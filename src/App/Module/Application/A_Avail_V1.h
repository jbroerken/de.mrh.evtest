...
// Project
#include "./A_LaunchSOA_V1.h"


class A_Avail_V1 : public EventModule
{
public:
    ...
    A_Avail_V1(std::shared_ptr<EventSender>& p_EventSender) : EventModule("MRH_EVENT_APP_AVAIL",
                                                                          MRH_EVENT_APP_AVAIL_S,
                                                                          false,
                                                                          p_EventSender)
    {
        MRH_Event* p_Event = MRH_EVD_CreateEvent(MRH_EVENT_APP_AVAIL_U, NULL, 0);

        if (p_Event == NULL)
        {
            throw MRH::AB::ModuleException(GetIdentifier(),
                                           "Failed to create MRH_EVENT_APP_AVAIL_U event!");
        }
    ...
    {
        return std::make_unique<A_LaunchSOA_V1>(p_EventSender);
    }
