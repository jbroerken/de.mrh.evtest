...
#ifndef A_LaunchSOAClear_V1_h
#define A_LaunchSOAClear_V1_h
...
// Project
#include "./A_LaunchSOATimer_V1.h"


class A_LaunchSOAClear_V1 : public EventModule
{
public:
    ...
    A_LaunchSOAClear_V1(std::shared_ptr<EventSender>& p_EventSender) : EventModule("MRH_EVENT_APP_LAUNCH_SOA_CLEAR",
                                                                                   ...
                                                                                   p_EventSender)
    {
        ...
        if (p_Event == NULL)
        {
            throw MRH::AB::ModuleException(GetIdentifier(),
                                           "Failed to create MRH_EVENT_APP_LAUNCH_SOA_CLEAR_U event!");
        }
    ...
    {
        return std::make_unique<A_LaunchSOATimer_V1>(p_EventSender);
    }
    ...
    bool EventValid(const MRH_Event* p_Event) noexcept override
    {
        if (p_Event->u32_Type != MRH_EVENT_APP_LAUNCH_SOA_CLEAR_S)
        {
            MRH::AB::Logger::Singleton().Log(MRH::AB::Logger::ERROR, GetIdentifier() +
                                                                     ": Wrong event type!",
                                             "A_LaunchSOAClear_V1.h", __LINE__);
            return false;
        }
