...
// Project
#include "./S_String_V1.h"


class S_Avail_V1 : public EventModule
{
public:
    ...
    S_Avail_V1(std::shared_ptr<EventSender>& p_EventSender) : EventModule("MRH_EVENT_SAY_AVAIL",
                                                                          MRH_EVENT_SAY_AVAIL_S,
                                                                          false,
                                                                          p_EventSender)
    {
        MRH_Event* p_Event = MRH_EVD_CreateEvent(MRH_EVENT_SAY_AVAIL_U, NULL, 0);

        if (p_Event == NULL)
        {
            throw MRH::AB::ModuleException(GetIdentifier(),
                                           "Failed to create MRH_EVENT_SAY_AVAIL_U event!");
        }
    ...
    {
        return std::make_unique<S_String_V1>(p_EventSender);
    }
