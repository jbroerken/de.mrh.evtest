...
#ifndef U_Custom_V1_h
#define U_Custom_V1_h
...
// Project
#include "../Application/A_Avail_V1.h"


class U_Custom_V1 : public EventModule
{
public:
    ...
    U_Custom_V1(std::shared_ptr<EventSender>& p_EventSender) : EventModule("MRH_EVENT_USER_CUSTOM_COMMAND",
                                                                           MRH_EVENT_USER_CUSTOM_COMMAND_S,
                                                                           false,
                                                                           p_EventSender)
    {
        MRH_Event* p_Event = MRH_EVD_CreateEvent(MRH_EVENT_USER_CUSTOM_COMMAND_U, NULL, 0);

        if (p_Event == NULL)
        {
            throw MRH::AB::ModuleException(GetIdentifier(),
                                           "Failed to create MRH_EVENT_USER_CUSTOM_COMMAND_U event!");
        }
    ...
    {
        return std::make_unique<A_Avail_V1>(p_EventSender);
    }
...
private:
    ...
    {
        if (p_Event->u32_Type != MRH_EVENT_USER_CUSTOM_COMMAND_S)
        {
            MRH::AB::Logger::Singleton().Log(MRH::AB::Logger::ERROR, GetIdentifier() +
                                                                     ": Wrong event type!",
                                             "U_Custom_V1.h", __LINE__);
            return false;
        }
